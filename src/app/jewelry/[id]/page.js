...
import { FaHeart, FaRegHeart, FaShoppingCart, FaArrowLeft, FaShieldAlt, FaTruck, FaUndo, FaCheck } from "react-icons/fa";
...
const resolveCategoryName = (categoryField, subcategoryField) => {
    ...
    const id = typeof categoryField === "string" ? categoryField : categoryField?._id || subcategoryField?.parent_id;
    const idMap = {
...
export default function ProductDetail({ params }) {
    ...
    const { calculatePrice, addToCart, toggleWishlist, isWishlisted, formatPrice } = useStore();
    
    const dispatch = useDispatch();
    const { selectedProduct: rawProduct, error: apiError } = useSelector((state) => state.products);
    ...
    useEffect(() => {
        if (rawProduct) {
            ...
            const mapped = {
                ...
                image: p.images && p.images[0] ? p.images[0] : "https://images.unsplash.com/photo-1599643477877-530eb83abc8e?w=800",
                description: p.description || "Ethically sourced certified diamond luxury masterpiece.",
    ...
    useEffect(() => {
        if (product && selectedMetal && selectedCarat) {
            ...
            const goldVal = (product.goldWeight || 0) * metalMultiplier * 75.00;
        ...
        return (
            <Layout>
                <div className="flex-1 flex flex-col justify-center items-center py-40 px-4 bg-white text-center space-y-6">
                    ...
                    <h2 className="font-serif text-lg md:text-xl uppercase tracking-widest text-neutral-800">Creations Atelier</h2>
                    <p className="text-[10px] md:text-xs text-neutral-400 max-w-sm uppercase tracking-wider leading-relaxed">
                        This item could not be retrieved from our vaults. It may have been sold or does not exist.
                    </p>
                    <Link href="/jewelry" className="px-6 py-3 bg-neutral-950 hover:bg-neutral-900 text-white text-[10px] font-bold uppercase tracking-widest transition-colors cursor-pointer">
                        Return to Catalog
                    </Link>
                </div>
            </Layout>
        ...
        return (
            <Layout>
                <div className="flex-1 flex flex-col justify-center items-center py-40 space-y-4 font-sans bg-white">
                    ...
                    <span className="text-[10px] text-neutral-400 font-bold uppercase tracking-[0.2em]">Loading Atelier Piece...</span>
                </div>
            </Layout>
        );
    }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                    ...
                    <div className="space-y-4">
                        <div className="aspect-square w-full rounded-2xl overflow-hidden bg-white border border-slate-100 shadow-xs relative">
                            <img 
                                src={product.image || "https://images.unsplash.com/photo-1599643477877-530eb83abc8e?w=800"} 
                                alt={product.title} 
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            {[1, 2, 3].map((num) => (
                                <div key={num} className="aspect-square rounded-xl overflow-hidden border border-slate-100 bg-white hover:border-primary/50 transition-all cursor-pointer">
                                    <img 
                                        src={product.image || "https://images.unsplash.com/photo-1599643477877-530eb83abc8e?w=400"} 
                                        alt={`Angle ${num}`} 
                    ...
                    <div className="text-left space-y-6 lg:pl-4">
                        <div className="space-y-2 border-b border-slate-100 pb-5">
                            <div className="flex justify-between items-center">
                                <span className="text-xs text-primary font-bold uppercase tracking-[0.25em]">{product.category}</span>
                            ...
                            <h1 className="text-2xl sm:text-3xl font-serif font-medium text-slate-900 tracking-wide">{product.title}</h1>
                            <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Stock ID: {product.id} • Collection Style: {product.style}</p>
                        </div>

                        {/* Pricing Calculator Output */}
                        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 space-y-2">
                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest block">Configured Atelier Pricing</span>
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">1. Select Metal Color & Purity</label>
                            ...
                            {product.diamondWeight && product.diamondWeight.length > 0 && (
                                <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">2. Select Diamond Carat weight</label>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-slate-100">
                            ...
                            >
                                <FaHeart className={wishlisted ? "text-white" : ""} /> {wishlisted ? "Saved" : "Save Favorite"}
                            </button>
                        </div>
                        ...
                        {successAdded && (
                            <div className="bg-neutral-50 border border-neutral-200 text-neutral-800 rounded-xl p-3.5 text-xs font-semibold flex items-center gap-2 animate-fade-in">
                                <FaCheck className="text-neutral-800" /> Specs added successfully. View in bag at checkout!
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-3 gap-4 border-t border-slate-100 pt-6">
                            {[
                                { text: "Lifetime Warranty", icon: <FaShieldAlt className="text-accent text-base" /> },
                                { text: "FedEx Free Shipping", icon: <FaTruck className="text-accent text-base" /> },
                                { text: "Easy Returns", icon: <FaUndo className="text-accent text-base" /> }
                            ].map((trust, idx) => (
                                <div key={idx} className="flex flex-col items-center text-center space-y-1">
                                    {trust.icon}
                                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">{trust.text}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-2 text-xs font-light leading-relaxed border-t border-slate-100 pt-6">
                            <h4 className="text-[10px] font-bold text-slate-800 uppercase tracking-widest">Atelier Craftsmanship Notes</h4>
                            <p className="text-slate-500">
                                {product.description} Each gemstone cluster is claw-set by hand under a microscope to ensure perfect optical reflection alignment and secure durability. Gold bands are forged and finished at standard hallmark certification quality.
                            </p>
                        </div>
