...
import { FaFilter, FaSearch, FaRing, FaTimes, FaChevronDown, FaChevronUp } from "react-icons/fa";
...
function CatalogContent() {
    ...
    const { items: rawProducts, loading: apiLoading } = useSelector((state) => state.products);
    ...
    const apiProducts = useMemo(() => {
        return rawProducts.map(p => {
            ...
            return {
                ...
                image: p.images && p.images[0] ? p.images[0] : "https://images.unsplash.com/photo-1599643477877-530eb83abc8e?w=600&fit=crop",
    ...
    const filteredJewelry = productsSource.filter((item) => {
        // Search filter
        if (search && !item.title.toLowerCase().includes(search.toLowerCase()) && !item.category.toLowerCase().includes(search.toLowerCase())) {
            return false;
        }
        ...
        if (selectedStyle && (!item.style || !item.style.toLowerCase().includes(selectedStyle.toLowerCase()))) {
            return false;
        }
        ...
        if (selectedCaratRange) {
            const hasMatchingCarat = item.diamondWeight && item.diamondWeight.some(w => {
    ...
    return (
        <div className="mx-auto w-full max-w-[1600px] px-4 sm:px-6 lg:px-8 py-12 relative">
            ...
            <div className="text-center space-y-2 mb-12">
                <span className="text-[9px] text-neutral-400 font-extrabold tracking-[0.3em] uppercase">High Jewelry Atelier</span>
                <h1 className="text-3xl lg:text-4xl font-serif font-light text-neutral-900 uppercase tracking-widest">
                    The Jewelry <span className="font-serif italic font-light lowercase">Collection</span>
                </h1>
            </div>
            ...
            <div className="flex items-center justify-between border-y border-neutral-100 py-3.5 px-2 mb-10 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-neutral-800">
                ...
                <div className="flex items-center gap-2">
                    <span className="text-neutral-400 font-medium tracking-widest">Sort</span>
                    <select 
                        value={sortOrder}
                ...
                <>
                    ...
                    <div className="fixed top-0 left-0 h-full w-80 bg-white shadow-2xl z-50 flex flex-col justify-between transform transition-transform duration-300 animate-slide-in-left">
                        ...
                        <div className="flex items-center justify-between px-6 py-5 border-b border-neutral-100">
                            <span className="text-xs font-bold tracking-[0.2em] text-neutral-800 uppercase">Filters</span>
                            <button 
                                onClick={() => setIsFilterDrawerOpen(false)}
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6 text-left">
                            ...
                            <div className="border-b border-neutral-100 pb-4">
                                ...
                                >
                                    ...
                                    {accordions.search ? <FaChevronUp size={8} /> : <FaChevronDown size={8} />}
                                </button>
                                {accordions.search && (
                            ...
                            <div className="border-b border-neutral-100 pb-4">
                                ...
                                >
                                    <span>Category</span>
                                    {accordions.category ? <FaChevronUp size={8} /> : <FaChevronDown size={8} />}
                                </button>
                                {accordions.category && (
                                    <div className="space-y-1 mt-2">
                                        {[
                                            ...
                                            { label: "Bracelets & Bangles", value: "Bracelet & Bangle" },
                            ...
                            <div className="border-b border-neutral-100 pb-4">
                                ...
                                >
                                    ...
                                    {accordions.metal ? <FaChevronUp size={8} /> : <FaChevronDown size={8} />}
                                </button>
                                {accordions.metal && (
                                    <div className="space-y-1 mt-2">
                                        {[
                            ...
                            <div className="border-b border-neutral-100 pb-4">
                                ...
                                >
                                    <span>Max Price</span>
                                    {accordions.price ? <FaChevronUp size={8} /> : <FaChevronDown size={8} />}
                                </button>
                                {accordions.price && (
                                    <div className="space-y-3 mt-2">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[10px] text-neutral-400 font-medium">Cap value:</span>
                                            <span className="text-xs font-bold text-neutral-800">{formatPrice(maxPrice)}</span>
                                        </div>
                                        <input 
                                            type="range" 
                                            min="500" 
                            ...
                            <div className="border-b border-neutral-100 pb-4">
                                ...
                                >
                                    ...
                                    {accordions.origin ? <FaChevronUp size={8} /> : <FaChevronDown size={8} />}
                                </button>
                                {accordions.origin && (
                                    <div className="space-y-1 mt-2">
                                        {[
                            ...
                            <div className="border-b border-neutral-100 pb-4">
                                ...
                                >
                                    ...
                                    {accordions.carat ? <FaChevronUp size={8} /> : <FaChevronDown size={8} />}
                                </button>
                                {accordions.carat && (
                                    <div className="space-y-1 mt-2">
                                        {[
            ...
            <div className="w-full">
                {apiLoading ? (
                    <div className="flex flex-col justify-center items-center py-32 space-y-4 text-center">
                        ...
                        <span className="text-[10px] text-neutral-400 font-bold uppercase tracking-[0.2em]">Loading Atelier Pieces...</span>
                    </div>
                    ...
                    <div className="flex flex-col justify-center items-center text-center py-32 px-4 bg-white border border-neutral-100 rounded-sm space-y-5 max-w-lg mx-auto">
                        <span className="font-serif italic text-3xl text-neutral-300">✦</span>
                        <h3 className="text-xs lg:text-sm font-bold text-neutral-800 uppercase tracking-widest font-sans">No matching creations found</h3>
