...
import { FaTimes, FaTrash, FaPlus, FaMinus, FaLock, FaGift, FaCheckCircle, FaCreditCard, FaUpload, FaMapMarkerAlt, FaQrcode, FaArrowRight } from "react-icons/fa";
import { useStore } from "@/context/StoreContext";
import confetti from "canvas-confetti";

export default function CartDrawer({ isOpen, onClose }) {
    const { 
        cart, updateQuantity, removeFromCart, appliedCoupon, 
        applyCouponCode, removeCoupon, getCartSubtotal, 
        getDiscountAmount, getCartTotal, checkoutOrder,
        region, getRegionDetails, formatPrice, formatConvertedPrice, getTaxAmount
    } = useStore();
    ...
    const getGrandTotal = () => {
        ...
        return getCartTotal() + Math.round(getShippingFee() * (shippingMethod === "Express Insured Delivery" ? feeMultiplier : 0));
    };

    const isKycRequired = region === "HK" && getCartTotal() >= 120000;
    const isKycCompleted = !isKycRequired || (kycFile && kycDocNum.trim().length > 0);
                ...
                <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col glass-drawer">
                    ...
                    <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                        <div className="flex flex-col text-left">
                            <h2 className="text-sm font-bold tracking-[0.2em] text-slate-800 uppercase">Shopping Cart</h2>
                            <span className="text-[10px] text-slate-400 font-semibold tracking-wider mt-0.5">
                                {cart.length === 0 ? "Your bag is empty" : `${cart.reduce((tot, i) => tot + i.quantity, 0)} Items Added`}
                            </span>
                        </div>
                    ...
                    {orderConfirmed ? (
                        <div className="flex-1 flex flex-col justify-start items-center text-center p-6 overflow-y-auto space-y-6">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Reference ID: {orderConfirmed.id}</p>
                            </div>
                            
                            <p className="text-xs text-slate-500 max-w-[320px] leading-relaxed font-light">
                                Thank you, <span className="font-semibold text-slate-700">{orderConfirmed.customerName}</span>. Your luxury inquiry has been received. Our concierge representative will contact you at <span className="font-semibold text-slate-700">{orderConfirmed.email}</span> shortly.
                            </p>

                            {/* Localized HK Payment Action on success screen */}
                            {region === "HK" && ["FPS", "PayMe", "AlipayHK", "WeChat Pay HK"].includes(orderConfirmed.paymentMethod) && (
                                <div className="border border-primary/20 bg-primary/5 rounded-2xl p-4 w-full text-left space-y-3">
                                    <h4 className="text-[11px] font-bold text-slate-800 flex items-center gap-1.5 uppercase tracking-wider">
                                        <FaQrcode className="text-primary" /> Scan or Authorize Local Payment
                                    </h4>
                                        ...
                                        <div className="space-y-2 text-center">
                                            <div className="flex justify-center p-2 bg-white rounded-xl border border-slate-100 max-w-[120px] mx-auto">
                                                <svg width="100" height="100" viewBox="0 0 100 100" className="text-slate-800">
                                                    ...
                                                    <path d="M5,5 h20 v20 h-20 z M10,10 h10 v10 h-10 z M75,5 h20 v20 h-20 z M80,10 h10 v10 h-10 z M5,75 h20 v20 h-20 z M10,80 h10 v10 h-10 z" fill="currentColor" />
                                                    ...
                                                    <rect x="32" y="42" width="36" height="16" rx="3" fill="#1A1A1A" />
                                                    <text x="50" y="53" fill="white" fontSize="8" fontWeight="black" textAnchor="middle">FPS</text>
                                                </svg>
                                            </div>
                                            <p className="text-[10px] text-slate-500 text-left font-light leading-relaxed">
                                                Please scan the FPS QR code using your local mobile banking app to authorize transfer of <strong>{formatConvertedPrice(orderConfirmed.totalAmount)}</strong> to Dndiamond Accounts.
                                            </p>
                                        </div>
                                    )}
                                    ...
                                    {orderConfirmed.paymentMethod === "PayMe" && (
                                        <div className="space-y-2 text-center">
                                            ...
                                            <p className="text-[10px] text-slate-500 text-left font-light leading-relaxed">
                                                Redirected successfully. If the application did not launch, please wire the total of <strong>{formatConvertedPrice(orderConfirmed.totalAmount)}</strong> directly using PayMe P2P code.
                                            </p>
                                        </div>
                                    )}

                                    {["AlipayHK", "WeChat Pay HK"].includes(orderConfirmed.paymentMethod) && (
                                        <div className="space-y-2 text-center">
                                            <div className="flex justify-center p-2 bg-white rounded-xl border border-slate-100 max-w-[120px] mx-auto">
                                                <svg width="100" height="100" viewBox="0 0 100 100" className="text-neutral-800">
                                                    <rect width="100" height="100" fill="white" />
                                                    <path d="M5,5 h20 v20 h-20 z M10,10 h10 v10 h-10 z M75,5 h20 v20 h-20 z M80,10 h10 v10 h-10 z M5,75 h20 v20 h-20 z M10,80 h10 v10 h-10 z" fill="currentColor" />
                                                    <rect x="35" y="5" width="5" height="15" fill="currentColor" />
                                                    <rect x="45" y="10" width="10" height="5" fill="currentColor" />
                                            ...
                                            <p className="text-[10px] text-slate-500 text-left font-light leading-relaxed">
                                                Please scan the QR code above with your mobile {orderConfirmed.paymentMethod} app to settle the amount of <strong>{formatConvertedPrice(orderConfirmed.totalAmount)}</strong>.
                                            </p>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 w-full text-left space-y-2">
                                <div className="flex justify-between text-xs">
                                    ...
                                    <span className="font-extrabold text-slate-800">{formatConvertedPrice(orderConfirmed.totalAmount)}</span>
                                </div>
                                <div className="flex justify-between text-xs">
                                    <span className="text-slate-400 font-medium">Payment Mode:</span>
                                    <span className="font-bold text-slate-600">{orderConfirmed.paymentMethod}</span>
                                </div>
                                <div className="flex justify-between text-xs">
                                    <span className="text-slate-400 font-medium">Logistics:</span>
                                    <span className="font-semibold text-slate-600">{orderConfirmed.shippingMethod}</span>
                                </div>
                                <div className="flex justify-between text-xs">
                                    <span className="text-slate-400 font-medium">Shipping Address:</span>
                                    <span className="font-medium text-slate-600 truncate max-w-[180px]">{orderConfirmed.address}</span>
                                </div>
                            </div>
                            <button 
                                onClick={handleReset}
                        ...
                        <div className="flex-1 flex flex-col justify-center items-center text-center p-8 space-y-4">
                            ...
                            <h3 className="text-sm font-bold text-slate-800 tracking-wider">Your bag is empty</h3>
                            <p className="text-xs text-slate-400 max-w-[240px] leading-relaxed font-light">
                                Explore our custom ring builders and loose GIA-certified diamonds to find your perfect pieces.
                            </p>
                            <button 
                                onClick={onClose}
                            ...
                            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
                                {!isCheckoutMode ? (
                                    ...
                                    cart.map((item, index) => (
                                        <div key={index} className="flex gap-4 p-3 rounded-xl border border-slate-100 hover:border-slate-200 transition-all bg-white">
                                            ...
                                            <div className="text-right shrink-0 flex flex-col justify-between">
                                                <span className="text-xs font-extrabold text-slate-900">{formatPrice(item.price * item.quantity)}</span>
                                                <span className="text-[9px] text-slate-400 font-medium">{formatPrice(item.price)} each</span>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    ...
                                    <form onSubmit={handlePlaceOrder} className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="space-y-1 text-left">
                                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">City / Territory</label>
                                                <input 
                                                    type="text" 
                                                    name="city" 
                                                    required
                                            ...
                                            <div className="space-y-1 text-left">
                                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Postal / ZIP Code</label>
                                                <input 
                                                    type="text" 
                                                    name="postalCode" 
                                                    required
                                        ...
                                        <div className="space-y-1 text-left">
                                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Shipping Method</label>
                                            <select 
                                                name="shippingMethod" 
                                            ...
                                            >
                                                {region === "HK" ? (
                                                    <>
                                                        <option value="Standard Courier Delivery">Standard Courier Delivery (Free)</option>
                                                        <option value="Showroom Pickup - Central">Showroom Pickup - Central, Hong Kong (Free)</option>
                                                    </>
                                                ) : (
                                                    <>
                                                        <option value="Standard Shipping">Standard Shipping (Free)</option>
                                                        <option value="Express Insured Delivery">Express Insured Delivery (Flat $25)</option>
                                                    </>
                                                )}
                                            </select>
                                            {region === "HK" && shippingMethod === "Showroom Pickup - Central" && (
                                                <div className="bg-primary/5 border border-primary/20 rounded-xl p-2.5 text-[9px] text-slate-600 font-medium leading-relaxed mt-1">
                                                    📍 <strong>Showroom Location:</strong> 18/F, Central Tower, 28 Queen's Road Central, Hong Kong.<br />
                                                    Hours: Mon-Sat 10:00 AM - 7:00 PM. Verification ID required on collection.
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="space-y-1 text-left">
                                            ...
                                            >
                                                {region === "HK" ? (
                                                    <>
                                                        <option value="Credit Card">Credit Card (Visa / Mastercard) & Apple Pay</option>
                                                        <option value="FPS">FPS (Faster Payment System QR Code)</option>
                                                        <option value="PayMe">PayMe (by HSBC Wallet)</option>
                                                        <option value="AlipayHK">AlipayHK Wallet</option>
                                                        <option value="WeChat Pay HK">WeChat Pay HK Wallet</option>
                                                        <option value="Bank Transfer">Local Bank Wire Transfer</option>
                                                    </>
                                                ) : (
                                                    <>
                                                        <option value="Credit Card">Credit Card (Visa / Mastercard / AMEX)</option>
                                                        <option value="UPI">UPI (Google Pay / Apple Pay)</option>
                                        ...
                                        {isCheckoutMode && region === "HK" && shippingInfo.paymentMethod === "FPS" && (
                                            <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50 text-center space-y-3 animate-fade-in text-left">
                                                <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 justify-center">
                                                    <FaQrcode className="text-neutral-800" /> Faster Payment System (FPS)
                                                </h4>
                                                ...
                                                <p className="text-[9px] text-slate-500 text-center leading-relaxed">
                                                    Scan with your HK banking app to settle <strong>{formatConvertedPrice(getGrandTotal())}</strong> instantly.
                                                </p>
                                            </div>
                                        )}

                                        {isCheckoutMode && region === "HK" && shippingInfo.paymentMethod === "PayMe" && (
                                            <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50 text-center space-y-3 animate-fade-in text-left">
                                                <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 justify-center">
                                                    <span className="h-4 w-4 bg-[#1A1A1A] rounded-full flex items-center justify-center text-[9px] font-black text-white">P</span>
                                                ...
                                                {payMePaid ? (
                                                    <div className="flex items-center gap-2 justify-center bg-neutral-50 border border-neutral-200 py-2.5 rounded-xl">
                                                        ...
                                                        <span className="text-[10px] text-neutral-800 font-bold uppercase tracking-wider">PayMe Authorized Successfully</span>
                                                    </div>
                                                ) : payMeRedirecting ? (
                                                    <div className="flex flex-col items-center justify-center py-2 space-y-2">
                                                        <div className="h-4 w-4 border-2 border-[#1A1A1A] border-t-transparent rounded-full animate-spin"></div>
                                                        <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Launching PayMe Wallet...</span>
                                                    </div>
                                                ) : (
                                                    <div className="space-y-2">
                                                        ...
                                                        >
                                                            Open PayMe to Pay {formatConvertedPrice(getGrandTotal())}
                                                        </button>
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        {isCheckoutMode && region === "HK" && ["AlipayHK", "WeChat Pay HK"].includes(shippingInfo.paymentMethod) && (
                                            <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50 text-center space-y-3 animate-fade-in text-left">
                                                <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 justify-center">
                                                ...
                                                <p className="text-[9px] text-slate-500 text-center leading-relaxed">
                                                    Scan with your {shippingInfo.paymentMethod} app to pay <strong>{formatConvertedPrice(getGrandTotal())}</strong>.
                                                </p>
                                            </div>
                                        )}

                                        {isCheckoutMode && shippingInfo.paymentMethod === "Credit Card" && (
                                            <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50 space-y-3 animate-fade-in text-left">
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2">
                                                        ...
                                                        <div className="space-y-1">
                                                            <label className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Security CVV</label>
                                                            <input 
                                                                type="text" 
                                                                placeholder="123" 
                                                                className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-[11px] focus:outline-none text-slate-800 font-semibold"
                                        ...
                                        {isKycRequired && (
                                            <div className="border border-neutral-200 bg-neutral-50/50 rounded-2xl p-4 text-left space-y-3 animate-fade-in">
                                                ...
                                                <p className="text-[9px] text-neutral-500 leading-relaxed font-light">
                                                    Anti-money laundering laws in HK require customer ID verification for transactions of <strong>HK$ 120,000</strong> or above. Please upload your document to proceed.
                                                </p>
                                                
                                                <div className="grid grid-cols-2 gap-2">
                                                    <div className="space-y-1">
                                                        <label className="text-[8px] font-bold text-slate-400 uppercase tracking-wider">Document Type</label>
                                                        <select
                                                            value={kycDocType}
                                                    ...
                                                    <div className="space-y-1">
                                                        <label className="text-[8px] font-bold text-slate-400 uppercase tracking-wider">Document Number</label>
                                                        <input
                                                            type="text"
                                                            required
                                                            value={kycDocNum}
                                                            onChange={(e) => setKycDocNum(e.target.value)}
                                                            placeholder={kycDocType === "HKID" ? "e.g. A123456(7)" : "e.g. G12345678"}
                                                ...
                                                <div className="space-y-1">
                                                    <label className="text-[8px] font-bold text-slate-400 uppercase tracking-wider">Upload Copy</label>
                                                    {kycSimulating ? (
                                                        <div className="bg-white border border-slate-200 rounded-xl p-3 text-center flex flex-col items-center justify-center space-y-1.5">
                                                            ...
                                                            <span className="text-[9px] text-slate-400 font-medium">Encrypting file securely...</span>
                                                        </div>
                                                    ) : !kycFile ? (
                                                        <div 
                                                        ...
                                                        >
                                                            ...
                                                            <span className="text-[8px] text-slate-500 font-bold uppercase tracking-wider">Tap to upload copy (ID/Passport)</span>
                                                            <input 
                                                                id="kyc-file-input" 
                                                                type="file" 
                                                                accept="image/*,.pdf" 
                                                        ...
                                                        <div className="flex items-center justify-between bg-neutral-50 border border-neutral-200 rounded-xl px-3 py-1.5 text-[9px] text-neutral-800 font-bold">
                                                            <span className="truncate max-w-[180px]">📄 {kycFile}</span>
                                                            <button 
                                                                type="button"
                                                                onClick={() => setKycFile("")}
                                                ...
                                                {kycFile && kycDocNum.trim().length > 0 && (
                                                    <div className="flex items-center gap-1.5 text-[9px] text-neutral-800 font-bold uppercase tracking-wider mt-1 pl-1">
                                                        <FaCheckCircle className="text-neutral-800" /> Identity copy verified and encrypted
                                                    </div>
                                                )}
                                            </div>
                                        )}
                            ...
                            <div className="border-t border-slate-100 bg-slate-50/50 px-6 py-5 space-y-4">
                                ...
                                {!isCheckoutMode && (
                                    <form onSubmit={handleApplyPromo} className="flex gap-2">
                                        ...
                                        <button type="submit" className="px-4 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-xs font-bold uppercase tracking-wider cursor-pointer">
                                            Apply
                                        </button>
                                    </form>
                                )}
                                ...
                                {promoError && <p className="text-[10px] text-neutral-800 font-semibold text-left">{promoError}</p>}
                                {promoSuccess && <p className="text-[10px] text-neutral-800 font-bold text-left">{promoSuccess}</p>}
                                ...
                                <div className="space-y-2 border-t border-slate-100 pt-3">
                                    <div className="flex justify-between text-xs text-left">
                                        ...
                                        <span className="font-extrabold text-slate-800">{formatConvertedPrice(getCartSubtotal())}</span>
                                    </div>
                                    {appliedCoupon && (
                                        <div className="flex justify-between text-xs text-neutral-800 text-left font-bold">
                                            <span className="font-medium">Discount</span>
                                            <span className="font-extrabold">-${formatConvertedPrice(getDiscountAmount())}</span>
                                        </div>
                                    )}
                                    
                                    <div className="flex justify-between text-xs text-left">
                                        <span className="text-slate-500 font-medium">Estimated Tax ({region === "HK" ? "0% GST/VAT" : `${region === "AU" ? "10% GST" : "15% GST"}`})</span>
                                    ...
                                    <div className="flex justify-between text-xs text-left">
                                        <span className="text-slate-500 font-medium">Shipping Fee ({shippingMethod})</span>
                                        <span className="font-bold text-neutral-800 uppercase">
                                            {getShippingFee() === 0 ? "Free" : formatConvertedPrice(Math.round(getShippingFee() * (region === "AU" ? 1.5 : 1.6)))}
                                        </span>
                                    </div>

                                    <div className="flex justify-between border-t border-slate-200/80 pt-3 text-left">
                                        <span className="text-sm font-bold text-slate-800 uppercase tracking-wider">Estimated Total</span>
                                ...
                                {!isCheckoutMode ? (
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        >
                                            {isKycRequired && !isKycCompleted ? "KYC Verification Required" : "Confirm Inquiry"}
                                        </button>
                                    </div>
                                )}
                            </div>
