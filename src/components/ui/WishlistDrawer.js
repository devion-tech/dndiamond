            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-3">
              {wishlist &&
                wishlist?.map((item) => (
                  <div
                    key={item.id}
                    className="flex gap-3 sm:gap-4 p-2.5 sm:p-3 rounded-xl border border-slate-100 hover:border-slate-200 transition-all bg-white relative group"
                  >
                    {item.image && item.image.includes("http") ? (
                      <img
                        src={item.image}
                        alt={item.title}
                        className="h-16 w-16 sm:h-20 sm:w-20 object-cover rounded-lg border border-slate-100 shrink-0"
                      />
                    ) : (
                      <div className="h-16 w-16 sm:h-20 sm:w-20 bg-slate-50 border border-slate-100 rounded-lg shrink-0 flex items-center justify-center text-primary font-bold text-lg">
                        {item.category ? item.category.charAt(0) : "J"}
                      </div>
                    )}
                    ...
                    <div className="flex-1 min-w-0 text-left flex flex-col justify-between">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <Link
                          ...
                          onClick={onClose}
                          className="bg-white text-neutral-900 border border-neutral-900 hover:bg-neutral-900 hover:text-white px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[9px] sm:text-[10px] font-medium uppercase tracking-wider flex items-center gap-1.5 cursor-pointer transition-all duration-300"
                        >
