...
import { initialJewelry, initialDiamonds, initialCoupons } from "../data/initialData";
import {
    initializeAuth,
...
export function StoreProvider({ children }) {
    ...
    const [goldPricePerGram, setGoldPricePerGram] = useState(75.00);
    ...
    const calculatePrice = (item, metalType, caratWeight) => {
        ...
        const discountModifier = item.discount ? (baseVal * (item.discount / 100)) : 0;
    ...
    const getRegionDetails = () => {
        switch (region) {
            case "AU":
                return { prefix: "A$", multiplier: 1.5, taxRate: 0.10, label: "Australia (AUD)" };
            case "NZ":
                return { prefix: "NZ$", multiplier: 1.6, taxRate: 0.15, label: "New Zealand (NZD)" };
            case "HK":
            default:
                return { prefix: "HK$", multiplier: 7.8, taxRate: 0.0, label: "Hong Kong (HKD)" };
        }
    };
    ...
    // --- Cart Redux Wrapper ---
    const addToCart = async (product, selectedMetal, selectedCarat, finalPrice) => {
    ...
    const registerUser = async (name, email, phone, password) => {
        const result = await dispatch(registerUserThunk({ name, email, phone, password }));
    ...
    const loginUser = async (email, password) => {
        ...
        if (loginUserThunk.fulfilled.match(result)) {
            ...
            await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:8080"}/api/cart/merge`, {
                method: "POST",
                headers: {
        ...
        return { success: false, message: result.payload || "Authentication failed" };
    };
    ...
    const applyCouponCode = (code) => {
        ...
        if (!coupon) return { success: false, message: "Invalid or inactive promo code." };
    ...
    const getCartSubtotal = () => {
        const baseSub = cart.reduce((total, item) => total + (item.price * item.quantity), 0);
