...
export const initializeAuth = createAsyncThunk(
    "auth/initialize",
    async (_, { dispatch }) => {
        if (typeof window === "undefined") return { token: null, user: null, guestId: "" };
    ...
    async ({ email, password }, { rejectWithValue }) => {
        ...
        try {
            ...
            if (data.success && data.token) {
                localStorage.setItem("praya_token", data.token);
                const userObj = data.user || { name: data.name || "User", email };
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(registerUser.fulfilled, (state) => {
                state.loading = false;
                state.successMessage = "Profile registered successfully. Please sign in.";
            })
