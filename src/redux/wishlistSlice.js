                ...
                state.items = action.payload.map(p => {
                    if (p && (p._id || p.id)) {
                        return {
                            ...
                            category: resolveCategoryName(p.category_id, p.subcategory_id) || p.category || "Ring",
                            image: p.images && p.images[0] ? p.images[0] : (p.image || "https://images.unsplash.com/photo-1599643477877-530eb83abc8e?w=600&fit=crop"),
                            ...
                            makingCharges: p.pricing?.additional_cost || p.makingCharges || 150,
